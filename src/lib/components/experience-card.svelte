		...
		>
			...
			>
				...
				<div class="relative mb-3">
					<p
						class="text-foreground/60 line-clamp-3 text-xs md:text-sm leading-relaxed transition-colors duration-300"
					>
						{item.description}
					</p>
				</div>
					...
					>
						<div in:fade={{ duration: 200 }} out:fade={{ duration: 150 }}>
							...
							{#if item.highlights && item.highlights.length > 0}
								<div>
									<h4 class="text-foreground top-0 -mt-2 mb-3 flex items-center py-2 font-semibold text-sm md:text-base">
									...
									<div class="grid gap-3">
										{#each item.highlights as highlight, idx}
											<div class="group flex items-start">
												...
												>
													<span class="text-accent text-xs md:text-sm font-medium">{idx + 1}</span>
												</div>
												<div class="flex-1">
													<p
														class="text-muted-foreground group-hover:text-foreground text-xs md:text-sm leading-relaxed transition-colors duration-200"
													>
														{highlight}
													</p>
												</div>
							...
							{#if item.tags && item.tags.length > 4}
								<div>
									<h4
										class="text-foreground sticky top-0 mt-2 mb-3 flex items-center py-2 font-semibold text-sm md:text-base"
									>
										<span class="from-primary to-accent mr-2 h-2 w-2 rounded-full bg-gradient-to-r"
										></span>
										Technologies & Skills
